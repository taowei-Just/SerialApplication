...
public class SerialHelper implements Function {
    ...
    public void open() {
        try {
            serialPort = new SerialPort(serialPath, baudrate, new Random().nextInt(4));
            inputStream = serialPort.getInputStream();
            outputStream = serialPort.getOutputStream();
            onMessageCall.onOpen("serial:"+serialPath +" baudrate:"+baudrate);
        } catch (Throwable e) {
            e.printStackTrace();
            onMessageCall.onInitFaile("serialPath:"+serialPath + " baudrate:" +baudrate+"\n"+e.toString());
            reInit();
        }
    }
    ...
    public void send(TobaccoProtocol.TobaccoCmdInfo info) {
        LogUtil.e(Gloable.MQ_TAG, " send " + StringUtils.bytes2HexString(info.getSendData()),true);
        ...
        if (outputStream != null) {
            ...
            try {
                ...
                if (onMessageCall != null)
                    onMessageCall.onSendFaile(e.toString());
                reInit();
            }
        } else {
            if (onMessageCall != null)
                onMessageCall.onSendFaile("outputStream==null");
            reInit();
        }
    }
