...
public class TobaccoProtocol {
    ...
    public static TobaccoCmdInfo praceData(TobaccoCmdInfo info , byte cmd) {
            ...
            for (int i = 0; i < data.length; i++) {
                if (data[i] == 0x48) {
                    ...
                    if (checkData(buff, b)) {
                        ...
                        if (tobaccoCmdInfo != null) {
                            tobaccoCmdInfo.setSendData(info.getSendData());
                            tobaccoCmdInfo.setNumber(StringUtils.byte2Int(info.getSendData()[2]));
                            tobaccoCmdInfo.setFlag(info.getFlag());
                        }
                        return tobaccoCmdInfo;
                    }
                }
...
//        rData[0] =info.getReceiveData()[0];
//        rData[1] =info.getReceiveData()[1];
//        info.setReceiveData(Verify.Make_CRC(rData));
    ...
    private static TobaccoCmdInfo outGoods(byte[] buff) {
        ...
        cmdInfo.setReceiveData(buff);
        cmdInfo.setResult(((int) buff[3]==1 || (int)buff[4]==1)? 1:0);
        cmdInfo.setNumber(StringUtils.byte2Int(buff[2]));
    ...
    public static  void  main(String[] a){
        ...
        for (int i=0 ;i<5 ;i++){
            System.err.println(""+i +"号灯开 " +StringUtils.bytes2HexString(getLightContorlCmd(i,true) ));
            try {
                Thread.sleep(100);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            System.err.println(""+i +"号灯关 " +StringUtils.bytes2HexString(getLightContorlCmd(i,false) ));
            try {
                Thread.sleep(100);
            } catch (InterruptedException e) {
                e.printStackTrace();
